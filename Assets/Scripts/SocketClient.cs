...
public class SocketClient  {
    ...
    private void ReceiveCallBack(IAsyncResult ar)
    {
        ...
        try
        {
            ...
            if (nLength > 0)
            {
                ...
                if (m_ReceiveMemoryStream.Length > DATA_HEAD_LENGTH)
                {
                    while (true)
                    {
                        ...
                        {
                            Debug.Log("客户端收到了一个" + currentFullMsgLen + "长度的消息");
            ...
            {
                Debug.LogWarning(string.Format("{0}断开连接", m_isClientClose ? "客户端断的" : "服务器断的"));
                Close(false);
            }
        }
        catch (Exception ex)
        {
            Debug.LogWarning(string.Format("{0}断开连接,{1}", m_isClientClose ? "客户端断的" : "服务器断的", ex));
            Close(false);
        }
    }
    /// <summary>
